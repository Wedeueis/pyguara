...
### Added
- **`FlockingSystem(..., groups=N)` staggers steering across ticks.** One group re-decides its heading per tick while *every* agent keeps integrating its position, so motion stays smooth and only the decision rate drops to 60/N Hz; the accumulated acceleration is scaled by `groups` so a staggered agent turns at the same rate rather than N times slower. At 3000 agents: 53.5 ms at `groups=1`, 24.9 at `groups=3`. It does not scale as 1/N — the hash rebuild, component resolution and position integration are paid every tick regardless, and that floor is what stops large group counts being interesting.
- **A measured performance baseline** (`tests/performance/`, `docs/guides/performance.md`). Two tiers: cheap *guards* that assert a complexity class by comparing the same operation at N and 4N in one process — which cancels machine speed out, so they are safe on any CI runner — and *sweeps* that assert nothing and produce the published tables via `make benchmark`. GL benchmarks use `moderngl.create_standalone_context()`, so they need no display and no SDL. The `performance` pytest marker was declared but used by zero tests; it now gates a real suite, wired into `make ci` as `perf-guard` and excluded from the coverage targets, since tracing every line costs more than the code being timed.
...
### Changed
- **Flocking is about 4x faster, and the crowd ceiling moved from ~300 to ~1000 agents at 60 Hz** (or ~2000 with steering staggered to 20 Hz). Four changes, each measured: the three steering behaviours were fused into a single pass over the neighbour list (-24%); flockmates are addressed by slot into parallel component lists instead of by entity id, removing a manager lookup plus two component lookups per neighbour (-11%); the inner loop does scalar arithmetic instead of `Vector2`, whose `__add__` probes `hasattr` twice before reconstructing and measures 6.5x slower than a float pair (-48%, the largest and the one nobody predicts); and `FlockingSystem` became the first *core* consumer of `register_cached_query` (-4.7%). Equivalence with the original `SteeringBehavior` composition is pinned by tests and was mutation-checked. **3000 fully-simulated boids at 60 Hz remains out of reach** — worth stating plainly, since it was the number a proposed demo assumed.
- **`SpatialHash` queries are about twice as fast.** `_candidates` allocated a set per query to deduplicate keys that cannot be duplicated — `insert` discards a key from its old cell before adding it to the new, so a key lives in exactly one cell. It is a generator now; the cell range is derived from `center ± radius` instead of via a `Rect`; and the distance test is squared, so confirming a candidate no longer calls `Vector2.distance_to`. 10.8 µs → 5.1 µs per query, still flat in the number of keys held.

### Fixed
- **`EntityPool.release()` was O(n), making a pool drain quadratic in its size.** It did an `in` scan followed by a `list.remove()`, both linear from index 0, so cost depended entirely on the order entities came back: releasing oldest-first found its match immediately, releasing newest-first scanned the whole list every time. Draining 3000 entities took 74 ms — four and a half frames — from a structure whose stated purpose is high-frequency spawn and despawn. The active set is now a dict keyed on entity id: **55× faster at 3000 and independent of release order**, with `get_active()`'s acquisition-order guarantee preserved because dicts are insertion-ordered. `release()` compares the stored object by identity rather than trusting the key, since `EntityManager` permits duplicate ids and two pools over one manager routinely hold identically-named entities.
- **The renderer's "10,000+ sprites at 60 FPS" claim was never measured** — it was copied from a migration backlog goal into the `ModernGLRenderer` docstring. Replaced with what the benchmarks actually show: a 20,000-sprite batch costs ~16 ms, of which four fifths is the Python instance-pack loop — a share that grows with sprite count — while `Batcher` upstream costs more than three times the whole GL path for the same batch. Also corrected a `REFACTOR_STATE.md` row that still listed `time_scale`/`paused` as missing after they landed in `22c8038`.
