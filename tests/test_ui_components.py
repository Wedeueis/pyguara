...
def test_label_rendering(mock_renderer):
...
def test_button_rendering(mock_renderer):
...
def test_button_interaction():
    ...
    processed = btn.handle_event("MOUSE_MOVE", Vector2(100, 100))
    assert not processed
    assert btn.state == UIElementState.NORMAL

    # 2. Mouse Move INSIDE
    processed = btn.handle_event("MOUSE_MOVE", Vector2(20, 20))
    assert processed
    assert btn.state == UIElementState.HOVERED

    # 3. Mouse Down INSIDE
    processed = btn.handle_event("MOUSE_DOWN", Vector2(20, 20), button=1)
    assert processed
    assert btn.state == UIElementState.PRESSED

    # 4. Mouse Up INSIDE (Click)
    mock_click = MagicMock()
    btn.on_click = mock_click

    processed = btn.handle_event("MOUSE_UP", Vector2(20, 20))
    assert processed
    assert btn.state == UIElementState.HOVERED
    ...
    processed = btn.handle_event("MOUSE_UP", Vector2(100, 100))
