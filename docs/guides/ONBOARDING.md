...
*   **Developer overlay (`pyguara/tools`):** An in-game debug overlay drawn on the engine's own `UIRenderer`. Press **F12** to toggle it, then function keys for individual tools (F1 performance, F2 inspector, F3 events, F4 physics, F5 hierarchy, F6 config, F7 assets, F8 help). The Hierarchy picks an entity and the Entity Inspector reflects its `dataclass` components live; the Assets browser lists resources and spawns entities from `DataResource` files. It is a debug surface, not a scene-authoring application (see #53).
...
*   **Tools infrastructure (`pyguara/tools`):** The `ToolManager` handles visibility, input routing, and keyboard shortcuts for every registered `Tool`. `SandboxApplication` registers the overlay above; a game can register its own `Tool` subclasses too.
