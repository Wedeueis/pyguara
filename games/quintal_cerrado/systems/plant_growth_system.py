...
`scenes.py`), and each species' `stage_seconds` from `species.py`.
...
class PlantGrowthSystem:
    ...
    def __init__(self, entity_manager: EntityManager, grid: GardenGrid) -> None:
        ...
        Args:
            ...
            grid: The plot whose `plant_at` occupancy this system walks.
        """
    ...
    def update(self, dt: float) -> None:
        ...
        for entity_id in self._grid.plant_at.values():
            ...
            if plant.growth_stage == "harvestable":
                # Terminal for now (harvesting is Phase 3's economy system)
